...
const ProductCard = ({ data }: { data: Array<SingleSlider> }) => {
  ...
  return (
    <>
      {processedData &&
        processedData.map((rows: SingleSlider, i: number) => (
          <Card
            key={i}
            className="max-w-[250px] max-h-[600px] hover:shadow-2xl"
            imgAlt="Apple Watch Series 7 in colors pink, silver, and black" 
          >
                    <img 
                    className="max-w-full max-h-[150px]"
                    src={rows.image} alt={rows.title}/>
            <a href={`/product?id=${rows._id}&title=${encodeURIComponent(rows.title)}`}>
            ...
            <div className="flex items-center justify-between">
              <span className="lg:text-2xl text-md md:text-lg font-bold text-gray-900 dark:text-white">
                $   {rows.price}
              </span>
              <a
                href={`/product?id=${rows._id}&title=${encodeURIComponent(rows.title)}`}
